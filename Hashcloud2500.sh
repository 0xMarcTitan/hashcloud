...
usage() {
  ...
  border "WARNING"
  echo "This installs Hashcat V6.1.1 which enables mode 2500 to be used"
  echo "If you want the latest version please use hashcloud.sh"
  border "Supported OS"
...
then
  ...
  cd hashcat && git checkout 61e8368 && git submodule update --init && make && make install
